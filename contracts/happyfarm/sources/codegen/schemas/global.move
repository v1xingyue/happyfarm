module happyfarm::global_schema {
	...
	public fun new(counter: u64, admin: address, field_price: u64, last_field_no: u64, init_user_socre: u64): GlobalData {
		GlobalData {
			counter, 
			admin, 
			field_price, 
	...
	public fun register(_obelisk_world: &mut World, _ctx: &mut TxContext) {
		let _obelisk_schema = new(0,@0x0000000,10,1000,200);
		world::add_schema<GlobalData>(_obelisk_world, SCHEMA_ID, _obelisk_schema);
		events::emit_set(SCHEMA_ID, none(), _obelisk_schema);
	}

	public(friend) fun set(_obelisk_world: &mut World,  counter: u64, admin: address, field_price: u64, last_field_no: u64, init_user_socre: u64) {
	...
	public fun get(_obelisk_world: &World): (u64,address,u64,u64,u64) {
